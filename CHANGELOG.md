...
## v1.2.2 (2026-07-01) — 识别精度与通知可靠性提升

### 变更概览

优化 Android 端短信验证码识别算法，修复 PC 端桌面通知失效问题。

### 详细变更

| 端 | 变更 |
|------|------|
| Android | 验证码识别新增**邻近度校验**：数字必须出现在关键词 40 字符以内才算验证码，解决公司短信"标题含关键字 + 正文含运单号"的误判 |
| Android | 新增**排除模式**：订单号/运单号/金额（￥x元）/7+位连续数字/电话号码格式自动跳过 |
| Android | 扩展关键词支持 `OTP`（境外 App 验证码） |
| PC | 桌面通知**三级降级策略**：plyer → PowerShell Toast → 控制台输出，解决 plyer 在 Windows 上频繁失效问题 |

### 版本

- Android: `versionCode` 7, `versionName` "1.2.2"
